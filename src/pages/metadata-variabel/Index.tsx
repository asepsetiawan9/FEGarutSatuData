...
const Index = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await http().get('/msvar');
  ...
  const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (keyword === '') {
      ...
      const filteredData = originalData.filter((item) =>
        item.nama_variabel.toLowerCase().includes(keyword)
      );
  ...
  return (
    ...
    >
      <BreadcrumbsWrapper>
        <div className="px-4">
          <h1 className="font-bold">Metadata Variabel</h1>
          ...
          <div className="my-4 flex gap-4">
            <div className="relative w-full">
              ...
              <input
                ...
                placeholder="Cari Metadata Variabel ..."
          ...
          <div className="flex flex-row justify-between ">
            <span className="pt-2.5 text-sm font-bold">
              {jumlahData || '0'} Jumlah Metadata variabel
            </span>
            ...
            <div className="space-y-4">
              {paginatedDataset.length === 0 ? (
                ...
                paginatedDataset.map((item) => (
                  <Link
                    key={item.id}
                    href={`/metadata-variabel/${item.slug}`}
